...
def inventario_infinito():
    ...
    if 0 <= x < len(player['inventario']): # Verificando se o índice está dentro do intervalo válido
        ...
        if item_desejado in funcoes_consumiveis:
            funcoes_consumiveis[item_desejado]()
            evnt_aleatorio()  # colocar a função infinito quando criar
        ...
        print("ESSE ITEM NÃO ESTÁ NO SEU INVENTARIO OU SEU INVENTARIO ESTÁ VAZIO.")
        evnt_aleatorio()  # colocar a função do infinito quando criar
    ...
    if p_vida > 0:
        while p_vida >0:
                ...
                if player['vida'] <=0:
                    ...
                    while player['vida'] > 0 :
                        perg = int(input("1 > Proximo Evento // 2 > usar item // 3 > Finalizar Run "))
                        ...
                        if perg == 3:
                            print(f"Obrigado por Jogar, sua pontuaçã foi >>> {player['pontos']}PONTOS  <<<")
                            save_score()
                            parar()
                            return 

def combate_orc_I():
    luta_infinita_mode(p=player['nome'],
